...
import xyz.yingshaoxo.atlantisspace.services.KillerService;
...
public class MainActivity extends AppCompatActivity {
    public static final String BROADCAST_CONTEXT_MENU_CLOSED = "xyz.yingshaoxo.atlantisspace.broadcast.CONTEXT_MENU_CLOSED";
    public static final String BROADCAST_SEARCH_FILTER_CHANGED = "xyz.yingshaoxo.atlantisspace.broadcast.SEARCH_FILTER_CHANGED";
    ...
    private void init() {
        if (mStorage.getBoolean(LocalStorageManager.PREF_IS_SETTING_UP) && !Utility.isWorkProfileAvailable(this)) {
            // System has already finished provisioning, but AtlantisSpace still
    ...
    private void bindServices() {
        ...
        ((AtlantisSpaceApplication) getApplication()).bindAtlantisSpaceService(new ServiceConnection() {
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                mServiceMain = IAtlantisSpaceService.Stub.asInterface(service);
                tryStartWorkService();
            }
    ...
    private void bindWorkService() {
        // Bind to the AtlantisSpaceService in work profile
    ...
    private void bindWorkServiceCb(ActivityResult result) {
        if (result.getResultCode() == RESULT_OK && result.getData() != null) {
            ...
            mServiceWork = IAtlantisSpaceService.Stub.asInterface(binder);
    ...
    private void startKiller() {
        // Start the sticky KillerService to kill the AtlantisSpaceService
...
//                pager.setCurrentItem(2);
    ...
    // remote (work) -> main
    // main -> remote (work)
    IAtlantisSpaceService getOtherService(boolean isRemote) {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        for(int i = 0; i < menu.size(); i++) {
            ...
            spanString.setSpan(new ForegroundColorSpan(Color.parseColor("#999999")), 0,     spanString.length(), 0); //fix the color to white
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (itemId == R.id.main_menu_freeze_all) {
            ...
            Utility.createLauncherShortcut(this, launchIntent,
                    ...
                    "atlantisspace-freeze-all", getString(R.string.freeze_all_shortcut));
            return true;
